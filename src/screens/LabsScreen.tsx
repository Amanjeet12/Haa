...
function toLab(item: ApiLab): Lab {
  ...
  const price = minimumTest?.final_price ?? Number(minimumTest?.offer_price ?? minimumTest?.normal_price ?? 0);
  return {
...
export function LabsScreen({ navigation }: Props) {
  ...
  const locations = useMemo<LabLocation[]>(() => zones.map(zone => ({
    id: zone.zone_id,
    name: zone.zone_name,
  ...
  const loadFirstPage = useCallback(async () => {
    ...
    try {
      ...
      setLabs([]);
      setLabsError(error instanceof Error ? error.message : 'Unable to load labs.');
    } finally {
  ...
  const loadMore = useCallback(async () => {
    if (!selectedZone || !pageMeta?.hasMore || pageMeta.nextStart === null || pageMeta.nextEnd === null || moreLoading) return;
    setMoreLoading(true);
    try {
      const page = await getLabsByZone(selectedZone.zone_id, pageMeta.nextStart, pageMeta.nextEnd);
      setLabs(current => {
        const known = new Set(current.map(item => item.id));
        return [...current, ...page.data.map(toLab).filter(item => !known.has(item.id))];
      });
      setPageMeta(page.meta);
    } finally {
  ...
  const visibleLabs = useMemo(() => labs.filter(lab => {
    const searchable = `${lab.name} ${lab.specialties.join(' ')} ${lab.address ?? ''}`.toLowerCase();
    const matchesQuery = searchable.includes(query.trim().toLowerCase());
    const matchesCertification = certification === 'All labs' || lab.certifications.includes(certification);
    const matchesPrice = advancedFilters.price === 'all' ||
      (advancedFilters.price === 'under400' && lab.price < 400) ||
      (advancedFilters.price === '400to500' && lab.price >= 400 && lab.price <= 500) ||
      (advancedFilters.price === 'above500' && lab.price > 500);
    return matchesQuery && matchesCertification && matchesPrice;
  }).sort((a, b) => advancedFilters.sort === 'az' ? a.name.localeCompare(b.name) : advancedFilters.sort === 'za' ? b.name.localeCompare(a.name) : 0), [advancedFilters, certification, labs, query]);
  ...
  return (
    <SafeAreaView edges={['top']} style={[styles.safe, { backgroundColor: theme.colors.background }]}>
      ...
      {labsLoading ? <View style={styles.state}><ActivityIndicator color={theme.colors.primary} size="large" /><AppText color={theme.colors.textMuted} style={styles.stateText}>Loading labs in {location}…</AppText></View> : labsError ? <View style={styles.state}><AppText style={styles.emptyTitle} weight="700">Could not load labs</AppText><AppText color={theme.colors.textMuted} style={styles.stateText}>{labsError}</AppText><Pressable onPress={loadFirstPage} style={[styles.retry, { backgroundColor: theme.colors.primary }]}><AppText color="#FFFFFF" style={styles.retryText} weight="700">Try again</AppText></Pressable></View> : <FlatList
        ...
        contentContainerStyle={[styles.list, { paddingBottom: Math.max(insets.bottom + 24, 36) }]}
        ListHeaderComponent={<View style={styles.heading}><AppText style={styles.title} weight="800">All labs</AppText><AppText color={theme.colors.primary} style={styles.count} weight="700">{pageMeta?.total ?? visibleLabs.length} lab{(pageMeta?.total ?? visibleLabs.length) === 1 ? '' : 's'} nearby</AppText></View>}
        ListEmptyComponent={<View style={styles.empty}><AppText style={styles.emptyTitle} weight="700">No labs found</AppText><AppText color={theme.colors.textMuted} style={styles.emptyText}>Try a different lab name or filter.</AppText></View>}
        ListFooterComponent={moreLoading ? <ActivityIndicator color={theme.colors.primary} style={styles.moreLoader} /> : undefined}
      />}
      <LocationSheet visible={locationOpen} selected={location} locations={locations} loading={locationsLoading} error={locationsError} onClose={() => setLocationOpen(false)} onSearchPress={() => { setLocationOpen(false); navigation.navigate('CitySearch'); }} onRetry={() => dispatch(fetchZones())} onSelect={chooseLocation} />
...
const styles = StyleSheet.create({
  safe: { flex: 1 }, list: { paddingHorizontal: 16 },
  heading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 2, marginBottom: 9 },
  title: { fontSize: 17, lineHeight: 22 }, count: { fontSize: 9, lineHeight: 12 },
  state: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 28 },
  stateText: { marginTop: 8, textAlign: 'center', fontSize: 11, lineHeight: 15 },
  empty: { alignItems: 'center', paddingVertical: 50 }, emptyTitle: { fontSize: 15, lineHeight: 20 }, emptyText: { marginTop: 4, fontSize: 11, lineHeight: 15 },
  retry: { marginTop: 14, borderRadius: 10, paddingHorizontal: 18, paddingVertical: 9 }, retryText: { fontSize: 10, lineHeight: 13 }, moreLoader: { marginVertical: 16 },
});
