...
export function AddressFormScreen({ navigation, route }: Props) {
  ...
  const [buildingName, setBuildingName] = useState(
    value?.buildingName ?? value?.building_name ?? '',
  );
  ...
  const [pincode, setPincode] = useState(
    location?.pincode ?? value?.pincode ?? '',
  );
  ...
  const save = async () => {
    ...
    if (
      ...
      setError(
        'Enter the address, city, state and pincode, then choose its location on the map.',
      );
      return;
    }
    ...
    try {
      if (existing)
        await updateCustomerAddress(token, existing.address_id, input);
  ...
  const remove = () => {
    ...
    Alert.alert(
      'Delete address?',
      'This saved collection address will be permanently removed.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setSaving(true);
            try {
              await deleteCustomerAddress(token, existing.address_id);
              navigation.goBack();
            } catch (deleteError) {
              setError(
          ...
          >
            ...
            >
              <View style={styles.field}>
                ...
                >
                  <MapView
                    ...
                    style={styles.map}
                    initialRegion={{
                      latitude: initialCoordinate?.latitude ?? 20.5937,
                      longitude: initialCoordinate?.longitude ?? 78.9629,
                      latitudeDelta: initialCoordinate ? 0.008 : 18,
                      longitudeDelta: initialCoordinate ? 0.008 : 18,
                    }}
                    onPress={event =>
                ...
                <AppText color={theme.colors.textMuted} style={styles.mapHint}>
                  {coordinate
                    ? `Pinned at ${coordinate.latitude.toFixed(
                        6,
                      )}, ${coordinate.longitude.toFixed(6)}`
                    : 'Tap the map to place the address pin.'}
                </AppText>
              </View>
              <AppText style={styles.label} weight="700">
                Address type
              ...
              <Field
                ...
                value={pincode}
                onChangeText={next =>
                  setPincode(next.replace(/\D/g, '').slice(0, 6))
                }
              ...
              >
                ...
                <View>
                  ...
                  >
                    Preselect this for future collections.
                  </AppText>
                </View>
              </Pressable>
            </View>
