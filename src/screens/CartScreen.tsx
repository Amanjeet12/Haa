...
export function CartScreen() {
  ...
  const groups = items.reduce<{ name: string; items: CommerceCartItem[] }[]>(
    (result, item) => {
      const name =
        item.vendorName ||
        (source === 'global' ? 'Global Store' : 'Quick Commerce');
  ...
  const loadAddresses = useCallback(async () => {
    ...
    try {
      const result = (await getCustomerAddresses(token)).filter(address =>
        Boolean(address.billing_address),
      );
      setAddresses(result);
      setSelectedAddressId(current =>
        result.some(address => address.address_id === current)
          ? current
          : result.find(address => address.billing_address?.isDefault)
              ?.address_id ??
            result[0]?.address_id ??
            null,
      );
  ...
  const addressSummary = (address: CustomerAddress) => {
    ...
    const street =
      addressLine(address) ||
      [
        details.flatNo || details.flat_no,
    ...
    return (
      [street, [city, pincode].filter(Boolean).join(' - ')]
        .filter(Boolean)
        .join(', ') ||
  ...
  const placeOrder = async () => {
    ...
    if (!token || !customer) {
      Alert.alert('Sign in required', 'Sign in before placing your order.');
      return;
    }
    if (!selectedAddress) {
      Alert.alert(
        'Choose an address',
        'Select a saved delivery address before placing your order.',
      );
    ...
    if (
      ...
      Alert.alert(
        'Incomplete address',
        'Please update the selected address with its street, city, pincode, and map location.',
        [
    ...
    if (
      ...
      Alert.alert(
        'Unavailable item',
        'This cart contains an item that cannot be ordered online.',
      );
      return;
    }
    ...
    if (source === 'ecommerce' && !zoneId) {
      Alert.alert(
        'Choose a location',
        'Select a delivery zone before placing your order.',
      );
      return;
    }
    ...
    try {
      ...
      const payload =
        error instanceof ApiError
          ? (error.payload as { msg?: string; message?: string } | undefined)
          : undefined;
      navigation
        .getParent<NativeStackNavigationProp<RootStackParamList>>()
        ?.navigate('OrderStatus', {
    ...
    >
      <SafeAreaView edges={['top']} style={styles.safe}>
        <View style={styles.hero}>
          ...
          {source ? (
            <AppText color={theme.colors.textMuted} style={styles.source}>
              {source === 'global' ? 'Global Store order' : 'E-commerce order'}
            </AppText>
          ) : null}
        </View>
        ...
        {!items.length ? (
          <View style={styles.empty}>
            ...
            <AppText color={theme.colors.textMuted} style={styles.emptyText}>
              E-commerce and Global Store items you add will appear here.
            </AppText>
          </View>
        ) : (
          <>
            ...
            >
              ...
              >
                ...
                >
                  <MapPin color={theme.colors.primary} size={19} />
                </View>
              ...
              <AppText style={styles.itemsHeading} weight="600">
                Your items
              </AppText>
              {groups.map(group => {
                const delivery = group.items.find(
                  ...
                  >
                    ...
                    >
                      <View style={styles.groupCopy}>
                        <AppText style={styles.groupName} weight="800">
                          {group.name}
                        </AppText>
                        <AppText
                          color={theme.colors.textMuted}
                          style={styles.groupSub}
                        >
                      ...
                      >
                        ...
                        <View style={styles.copy}>
                          <AppText
                            ...
                            weight="800"
                          >
                            {(item.brand || group.name).toUpperCase()}
                          </AppText>
                          <AppText
                            numberOfLines={2}
                            style={styles.name}
              ...
              <View style={styles.billCard}>
                <AppText color="#FFFFFF" style={styles.billTitle} weight="600">
                  Bill details
                </AppText>
                <View style={styles.billRow}>
                  <AppText color="#B7C3D1" style={styles.billLabel}>
                    Item total
                  </AppText>
                  <AppText color="#B7C3D1" style={styles.billValue}>
...
const styles = StyleSheet.create({
  ...
  source: { marginTop: 4, fontSize: 9, lineHeight: 12 },
  empty: {
    flex: 1,
    alignItems: 'center',
  ...
  groupCopy: { flex: 1 },
  groupName: { fontSize: 14, lineHeight: 18 },
  groupSub: { marginTop: 4, fontSize: 9, lineHeight: 12 },
  deliveryBadge: { borderRadius: 12, paddingHorizontal: 9, paddingVertical: 7 },
  deliveryText: { fontSize: 9, lineHeight: 12 },
  item: {
    minHeight: 112,
    padding: 13,
    flexDirection: 'row',
  ...
  imageWrap: { width: 66, height: 76, borderRadius: 17, overflow: 'hidden' },
  image: { width: '100%', height: '100%' },
  copy: { flex: 1 },
  brand: { fontSize: 8, lineHeight: 11, letterSpacing: 0.5 },
  name: { marginTop: 4, fontSize: 12, lineHeight: 16 },
  detail: { marginTop: 4, fontSize: 9, lineHeight: 13 },
  ...
  quantityText: { minWidth: 12, textAlign: 'center', fontSize: 11 },
  groupFooter: {
    minHeight: 39,
    borderTopWidth: 1,
    paddingHorizontal: 14,
  ...
  footerText: { fontSize: 10, lineHeight: 14 },
  billCard: {
    marginTop: 12,
    padding: 20,
    borderRadius: 22,
  ...
  billTitle: { marginBottom: 13, fontSize: 18, lineHeight: 23 },
  billRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  billLabel: { fontSize: 12, lineHeight: 17 },
  billValue: { fontSize: 12, lineHeight: 17 },
  billDivider: {
    height: 1,
    backgroundColor: '#365065',
    marginTop: 6,
    marginBottom: 13,
  },
  billTotalRow: {
  ...
  billTotal: { fontSize: 17, lineHeight: 22 },
  checkout: {
    position: 'absolute',
    left: 0,
    right: 0,
  ...
  checkoutText: { fontSize: 12, lineHeight: 16 },
  checkoutAmount: { fontSize: 13, lineHeight: 17 },
  addressCard: {
    minHeight: 68,
    borderWidth: 1,
    borderRadius: 17,
  ...
  addressText: { marginTop: 3, fontSize: 10, lineHeight: 14 },
  changeText: { fontSize: 11, lineHeight: 14 },
});
