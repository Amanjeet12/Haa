...
export function CitySearchScreen({ navigation }: Props) {
  ...
  const matchingZones = useMemo(() => {
    ...
    return value
      ? zones.filter(zone => zone.zone_name.toLowerCase().includes(value))
      : zones;
  }, [query, zones]);
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted)
          setMessage('City search is unavailable. Please try again.');
      } finally {
  ...
  const selectSuggestion = async (suggestion: PlaceSuggestion) => {
    setChecking(suggestion.placeId);
    setMessage(null);
    try {
...
const styles = StyleSheet.create({
  ...
  status: { flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 4 },
  statusText: { fontSize: 8, lineHeight: 10 },
  secondary: { marginTop: 2, fontSize: 9, lineHeight: 12 },
  message: {
    marginTop: 18,
    borderRadius: 14,
    padding: 15,
  ...
  messageText: { flex: 1, fontSize: 11, lineHeight: 15 },
});
