...
export function FamilyMemberFormScreen({ navigation, route }: Props) {
  ...
  const [relation, setRelation] = useState(member?.relation.toLowerCase() ?? '');
  ...
  const choosePhoto = async () => {
    ...
    if (asset?.uri) {
      setPhotoFile({ uri: asset.uri, type: asset.type, fileName: asset.fileName });
  ...
  const save = async () => {
    ...
    if (!name.trim() || !relation || !gender || !Number.isInteger(numericAge) || numericAge < 0 || numericAge > 120) {
  ...
  const confirmDelete = () => {
    ...
    Alert.alert('Delete family member?', `${member.name} will be removed from your saved profiles.`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            setError(deleteError instanceof Error ? deleteError.message : 'Unable to delete family member.');
            setSaving(false);
          }
        },
      },
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            <Pressable onPress={choosePhoto} style={styles.photoWrap}>
              <View style={[styles.photo, { backgroundColor: theme.colors.primarySoft, borderColor: theme.colors.border }]}>
                {photoUrl ? <Image source={{ uri: photoUrl }} style={styles.photoImage} /> : <AppText color={theme.colors.primary} style={styles.photoInitial} weight="800">{name.trim().charAt(0).toUpperCase() || '?'}</AppText>}
              </View>
              <View style={[styles.camera, { backgroundColor: theme.colors.primary }]}><Camera color="#FFFFFF" size={15} /></View>
            </Pressable>
            <AppText color={theme.colors.textMuted} style={styles.photoHint}>Tap to add a profile photo</AppText>
            ...
            <View style={[styles.form, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              ...
              <Field label="Age *" value={age} onChangeText={value => setAge(value.replace(/\D/g, '').slice(0, 3))} placeholder="Age" keyboardType="number-pad" />
              <Choice label="Gender *" options={genders} value={gender} onChange={setGender} />
              <Choice label="Relationship *" options={relations} value={relation} onChange={setRelation} />
              <Field label="Phone number" value={phone} onChangeText={value => setPhone(value.replace(/[^+\d]/g, '').slice(0, 14))} placeholder="Optional phone number" keyboardType="phone-pad" />
              <Pressable onPress={() => setIsDefault(value => !value)} style={styles.defaultRow}>
                <View style={[styles.checkbox, { borderColor: isDefault ? theme.colors.primary : theme.colors.border, backgroundColor: isDefault ? theme.colors.primary : theme.colors.surface }]}>{isDefault && <AppText color="#FFFFFF" style={styles.check} weight="800">✓</AppText>}</View>
                <View style={styles.grow}><AppText style={styles.defaultTitle} weight="700">Make default member</AppText><AppText color={theme.colors.textMuted} style={styles.defaultText}>Use this profile first when assigning tests.</AppText></View>
              </Pressable>
            </View>
            {!!error && <AppText color={theme.colors.danger} style={styles.error}>{error}</AppText>}
            <Pressable disabled={saving} onPress={save} style={[styles.save, { backgroundColor: saving ? theme.colors.border : theme.colors.primary }]}><AppText color="#FFFFFF" style={styles.saveText} weight="800">{saving ? 'Saving…' : member ? 'Save changes' : 'Add family member'}</AppText></Pressable>
            {!!member && <Pressable disabled={saving} onPress={confirmDelete} style={[styles.delete, { borderColor: theme.colors.primary }]}><Trash color={theme.colors.primary} size={16} /><AppText color={theme.colors.primary} style={styles.deleteText} weight="800">Delete family member</AppText></Pressable>}
          </ScrollView>
        </KeyboardAvoidingView>
      </SafeAreaView>
    </View>
...
function Field(props: { label: string; value: string; onChangeText: (value: string) => void; placeholder: string; keyboardType?: 'default' | 'number-pad' | 'phone-pad' }) { const { theme } = useAppTheme(); return <View style={styles.field}><AppText style={styles.label} weight="700">{props.label}</AppText><TextInput value={props.value} onChangeText={props.onChangeText} placeholder={props.placeholder} placeholderTextColor={theme.colors.textMuted} keyboardType={props.keyboardType} style={[styles.input, { color: theme.colors.text, borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceMuted }]} /></View>; }
