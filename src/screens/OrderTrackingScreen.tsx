  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const updated = orders.find(
        item =>
          item.order_id === route.params.order.order_id ||
          (item.order_no && item.order_no === route.params.order.order_no),
      );
  ...
  const stages = [
    {
      ...
      time: order.createdAt || order.created_at,
    },
    {
      label: 'Confirmed',
      done:
    ...
    },
    {
      label: 'Packed',
        ...
        ['packed', 'out_for_delivery', 'delivered'].includes(status),
      time: order.packed_at,
    },
    {
      label: 'On the way',
      done:
    ...
    {
      label: 'Delivered',
      done: Boolean(order.delivered_at) || status === 'delivered',
    ...
    >
      ...
      >
        ...
        <AppText color={theme.colors.textMuted} style={styles.subtitle}>
          Pull down to refresh the latest order status.
        </AppText>
        {error ? (
          ...
          {stages.map((stage, index) => (
            <View key={stage.label} style={styles.stage}>
              ...
              <View style={styles.stageCopy}>
                <AppText
                  color={
                    stage.done ? theme.colors.text : theme.colors.textMuted
                  }
                  style={styles.stageLabel}
                  weight={stage.done ? '800' : '500'}
                >
                  {stage.label}
                </AppText>
              ...
              {index === stages.length - 1 ? null : (
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        stage.done && stages[index + 1].done
                          ? theme.colors.success
                          : theme.colors.border,
                    },
                  ]}
        ...
        <AppText style={styles.productsTitle} weight="800">
          Products in this order
        </AppText>
        {order.items?.map((item, index) => {
          const image =
            item.product_image?.url || item.product_snapshot?.images?.[0]?.url;
            ...
            >
              ...
              >
                {image ? (
                  ...
                  <Package color={theme.colors.primary} size={25} />
                )}
              </View>
              <View style={styles.itemCopy}>
                <AppText style={styles.itemName} weight="800">
                ...
                {Number.isFinite(lineTotal) ? (
                  <AppText style={styles.amount} weight="800">
        ...
        {Number.isFinite(total) ? (
          <View style={styles.totalRow}>
            <AppText style={styles.totalLabel} weight="800">
              Order total
            </AppText>
            <AppText style={styles.totalLabel} weight="800">
...
const styles = StyleSheet.create({
  productsTitle: { marginTop: 21, fontSize: 15, lineHeight: 19 },
  viewDetails: { marginTop: 7, fontSize: 9, lineHeight: 12 },
  totalRow: {
    flexDirection: 'row',
  ...
  content: { padding: 17, paddingBottom: 32 },
  eyebrow: { fontSize: 8, lineHeight: 12, letterSpacing: 0.7 },
  title: { marginTop: 4, fontSize: 23, lineHeight: 28 },
  ...
  sectionTitle: { marginBottom: 14, fontSize: 15, lineHeight: 19 },
  stage: {
    minHeight: 49,
    flexDirection: 'row',
  ...
  dot: { width: 14, height: 14, borderWidth: 2, borderRadius: 7, marginTop: 2 },
  connector: { position: 'absolute', left: 6, top: 17, width: 2, height: 29 },
  stageCopy: { flex: 1 },
  stageLabel: { fontSize: 11, lineHeight: 16 },
  stageTime: { marginTop: 2, fontSize: 9, lineHeight: 12 },
  itemCard: {
    borderWidth: 1,
    borderRadius: 17,
    marginTop: 12,
  ...
  itemDetail: { marginTop: 4, fontSize: 9, lineHeight: 12 },
  amount: { marginTop: 6, fontSize: 12, lineHeight: 16 },
  loader: { marginTop: 14 },
});
