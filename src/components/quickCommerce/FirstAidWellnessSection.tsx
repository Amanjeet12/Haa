...
const products = [
  { brand: 'HAA CARE', name: 'Everyday First Aid\nKit', detail: '6 essential items', discount: '15% OFF', price: '₹299', oldPrice: '₹349', image: images.careImage, seller: 'MediQuick · 3 prices' },
  { brand: 'DAILY WELLNESS', name: 'ORS Hydration\nSachets', detail: 'Pack of 5', discount: '', price: '₹170', oldPrice: '', image: images.onboardingCards, seller: 'CarePoint · 4 prices' },
    ...
    <View style={styles.root}>
      <View style={styles.header}>
        <View style={styles.headerCopy}>
          <AppText style={styles.heading} weight="800">First aid & wellness</AppText>
          <AppText color={theme.colors.textMuted} style={styles.subtitle}>Daily health essentials</AppText>
        </View>
        <Pressable onPress={onSeeAll}><AppText color={theme.colors.primary} style={styles.viewAll} weight="800">View all</AppText></Pressable>
      </View>

      <ScrollView contentContainerStyle={styles.productRow} horizontal showsHorizontalScrollIndicator={false}>
        {products.map(product => (
          <View key={product.name} style={[styles.card, { width: productCardWidth, backgroundColor: theme.colors.surface, borderColor: theme.colors.border, shadowColor: theme.colors.shadow }]}>
            ...
            <View style={styles.body}>
              ...
              <AppText numberOfLines={2} style={styles.name} weight="800">{product.name}</AppText>
              ...
              <AppText color={theme.colors.success} numberOfLines={1} style={styles.seller} weight="700">● {product.seller}</AppText>
      ...
      <Pressable onPress={onSeeAll} style={[styles.seeAll, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
        <View style={styles.thumb}><Image resizeMode="cover" source={images.careImage} style={styles.image} /></View>
        <AppText style={styles.seeAllText} weight="800">See all First aid & wellness</AppText>
...
const styles = StyleSheet.create({
  ...
  card: { padding: 5, borderWidth: 1, borderRadius: 16, elevation: 2, shadowOpacity: 0.08, shadowRadius: 6, shadowOffset: { width: 0, height: 3 } },
  art: { aspectRatio: 1.52, overflow: 'hidden', borderRadius: 11, backgroundColor: '#DCECEF' },
  image: { width: '100%', height: '100%' },
  discount: { position: 'absolute', left: 8, top: 8, borderRadius: 8, paddingHorizontal: 7, paddingVertical: 4 },
  ...
  name: { minHeight: 32, marginTop: 3, fontSize: 12, lineHeight: 15, letterSpacing: -0.35 },
  ...
  add: { width: '41%', minWidth: 36, height: 34, borderRadius: 10, alignItems: 'center', justifyContent: 'center', elevation: 3, shadowColor: '#DF1F2D', shadowOpacity: 0.18, shadowRadius: 5, shadowOffset: { width: 0, height: 3 } },
  addText: { fontSize: 9, lineHeight: 11 },
  seeAll: { height: 48, marginHorizontal: 18, marginTop: 11, borderWidth: 1, borderRadius: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, paddingHorizontal: 14 },
