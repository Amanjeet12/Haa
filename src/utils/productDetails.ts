...
export function toProductDetails(product: Product, source: 'global' | 'ecommerce' = 'global'): HomeStackParamList['ProductDetails']['product'] {
  ...
  return {
    ...
    discount: product.discount_percentage > 0 ? `${Math.round(product.discount_percentage)}% OFF` : undefined,
    seller: product.attributes?.brand || product.vendor?.business_name || 'HAA HEALTH',
